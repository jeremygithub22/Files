...
{
    public partial class frmLWLSrep : Form
    {
        public frmLWLSrep()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                if (cboSearchby.Text == "Employee ID")
                {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Employee does not exist.", "Without Locator Slip Report", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        void perDept()
        {
            ...
            {
                conn.SetConstr();
                SQL = "select a.*,concat(b.Surname,', ',b.Firstname,' ',b.Middlename)as Name,c.Department as Office from tblwls a left join tblpi b on a.EmpID=b.EmpID left join (select * from (select * from tblwe order by year(str_to_date(`From`,'%m/%d/%Y')) desc,month(str_to_date(`From`,'%m/%d/%Y')) desc,day(str_to_date(`From`,'%m/%d/%Y')) desc)we group by EmpID) c on a.EmpID=c.EmpID where c.Department=@Department and a.DateRec>=@DateRecFrom and a.DateRec<=@DateRecTo";
                da = new MySqlDataAdapter(SQL, Connection.Conn);
                dsReports.tblwls.Clear();
                da.SelectCommand.Parameters.Add("@DateRecFrom", MySqlDbType.Date).Value = dtpFrom.Value;
                da.SelectCommand.Parameters.Add("@DateRecTo", MySqlDbType.Date).Value = dtpTo.Value;
                da.SelectCommand.Parameters.Add("@Department", MySqlDbType.VarChar).Value = txtSearch.Text;
                da.Fill(dsReports.tblwls);
                Connection.Conn.Close();
                if (dsReports.tblwls.Count != 0)
                {
                    LoadSet();
        ...
        void perDisplay()
        {
            ...
            {
                conn.SetConstr();
                SQL = "select a.*,concat(b.Surname,', ',b.Firstname,' ',b.Middlename)as Name,c.Department as Office from tblwls a left join tblpi b on a.EmpID=b.EmpID left join (select * from (select * from tblwe order by year(str_to_date(`From`,'%m/%d/%Y')) desc,month(str_to_date(`From`,'%m/%d/%Y')) desc,day(str_to_date(`From`,'%m/%d/%Y')) desc)we group by EmpID) c on a.EmpID=c.EmpID where a.EmpID=@EmpID and a.DateRec>=@DateRecFrom and a.DateRec<=@DateRecTo";
                da = new MySqlDataAdapter(SQL, Connection.Conn);
                dsReports.tblwls.Clear();
        ...
        void Display()
        {
            ...
            {
                conn.SetConstr();
                SQL = "select a.*,concat(b.Surname,', ',b.Firstname,' ',b.Middlename)as Name,c.Department as Office from tblwls a left join tblpi b on a.EmpID=b.EmpID left join (select * from (select * from tblwe order by year(str_to_date(`From`,'%m/%d/%Y')) desc,month(str_to_date(`From`,'%m/%d/%Y')) desc,day(str_to_date(`From`,'%m/%d/%Y')) desc)we group by EmpID) c on a.EmpID=c.EmpID where a.DateRec>=@DateRecFrom and a.DateRec<=@DateRecTo";
                da = new MySqlDataAdapter(SQL, Connection.Conn);
                dsReports.tblwls.Clear();
                da.SelectCommand.Parameters.Add("@DateRecFrom", MySqlDbType.Date).Value = dtpFrom.Value;
                da.SelectCommand.Parameters.Add("@DateRecTo", MySqlDbType.Date).Value = dtpTo.Value;
        ...
        {
            if (dsReports.tblwls.Rows.Count != 0)
            {
                frmAddWLS.rv = rvDisplay;
                frmAddWLS wls = new frmAddWLS();
                wls.ShowDialog();
            }
        }
    }
